...
func StatusPane() string {
	systemInformation := components.StatusComponent(
		...
		fmt.Sprintf("CPU | %s", components.StreamlineTemperatureChart(system.CPU.Temperature())),
		fmt.Sprintf("GPU | %s", components.StreamlineTemperatureChart(system.GPU.Temperature())),
